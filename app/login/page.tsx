...
const LoginPage = () => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    if (!resetPassword) {
      setError('Password reset is not configured on the server context.');
      setLoading(false);
      return;
    }
    ...
    setSuccess('Recovery transmission sent. Check your secure terminal (email).');
    setLoading(false);
  };
    ...
    <div className='min-h-screen bg-soft flex items-center justify-center p-4 md:p-6 font-luckiest text-foreground overflow-hidden'>
      ...
      >
        <div className='grid grid-cols-1 md:grid-cols-2'>
          {/* LEFT PANEL - Adapts perfectly on mobile, matching SignUp wrapper patterns */}
              ...
              {mode === 'login' ? (
                  ...
                  <form className='space-y-4' onSubmit={handleLogin}>
                    ...
                    <div className='space-y-2'>
                      <div className='flex justify-between items-end mx-1'>
                        <label className='block uppercase text-lg'>
                          Secret (Password)
                        </label>
                        <button
                          type='button'
                ...
                >
                  <form className='space-y-4' onSubmit={handleForgotPassword}>
                    <div className='space-y-2'>
                      ...
                      <div className='relative group'>
                        <div className='absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none'>
                          <MailIcon
                            size={18}
                            className='group-focus-within:text-secondary transition-colors'
                          />
                        </div>
                        <input
                          type='email'
                    ...
                    >
                      <ArrowLeftIcon size={14} /> Back to standard entry
                    </button>
                  </form>
                </motion.div>
              )}
