...
import {
  ArrowLeftIcon, TrophyIcon, TargetIcon, ZapIcon,
  CalendarIcon, PieChartIcon, DownloadIcon, TrendingUpIcon,
  BarChart3Icon, LogOutIcon, XIcon, StarIcon, ShieldCheckIcon, BarChart2Icon,
} from 'lucide-react';
...
const PremiumModal = ({ onClose }: { onClose: () => void }) => (
  <AnimatePresence>
    ...
    >
      ...
      >
        {/* Header */}
        <div className='bg-primary p-6 relative text-white text-center border-b-4 border-black'>
          <button
            onClick={onClose}
            className='absolute top-4 right-4 bg-white/20 hover:bg-white/40 border-2 border-white rounded-xl p-1 transition-colors cursor-pointer'
          >
            <XIcon size={18} className='text-white' />
          </button>
          <div className='flex justify-center mb-3'>
            <div className='bg-white/20 border-2 border-white p-3 rounded-2xl'>
              <StarIcon size={36} className='text-yellow-300' />
            </div>
          </div>
          <h2 className='text-3xl uppercase tracking-wide'>Premium</h2>
          <p className='text-white/80 text-sm uppercase mt-1'>Unlock Commander Tier</p>
        </div>

        {/* Features */}
        <div className='p-6 space-y-4'>
          {[
            { icon: <DownloadIcon size={20} />, text: 'Export Mission Debrief as PDF' },
            { icon: <BarChart2Icon size={20} />, text: 'Advanced Stats & Analytics' },
            { icon: <ShieldCheckIcon size={20} />, text: 'Priority Support' },
            { icon: <StarIcon size={20} />, text: 'Exclusive Commander Badge' },
          ].map((f, i) => (
            <div key={i} className='flex items-center gap-3 bg-soft border-2 border-black p-3 rounded-xl'>
        ...
        <div className='px-6 pb-6 space-y-3'>
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className='w-full bg-primary text-white border-4 border-black py-4 rounded-2xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] uppercase text-xl cursor-pointer'
          >
            Upgrade — $4.99 / mo
          </motion.button>
          <button
            onClick={onClose}
            className='w-full text-center text-sm opacity-50 uppercase hover:opacity-80 transition-opacity cursor-pointer'
          >
            Maybe later
          </button>
        </div>
...
const Dashboard = () => {
  ...
  const [historyData, setHistoryData] = useState<{ date: string; total: number; completed: number; percent: number; isToday: boolean }[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const todayQuests = quests.filter(q => new Date(q.created_at).toLocaleDateString() === todayStr);
      ...
      setStats({ total: totalToday, completed: completedToday, rate: totalToday > 0 ? Math.round((completedToday / totalToday) * 100) : 0, streak, xp });
    };

    fetchData();
  }, [user, supabase]);

  const getBarDecoration = (dayData: { percent: number; isToday: boolean } | undefined, barHeight: number, type: 'weekly' | 'monthly') => {
    ...
    return {
      ...
      backgroundImage: dayData ? `linear-gradient(45deg, rgba(255,255,255,0.2) 25%, transparent 25%, transparent 50%, rgba(255,255,255,0.2) 50%, rgba(255,255,255,0.2) 75%, transparent 75%, transparent)` : 'none',
  ...
  return (
    <motion.div
      initial='initial' animate='animate' variants={containerVars}
    ...
    >
      ...
      <div className='max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center mb-10 gap-6'>
        <Link href='/'>
          <motion.div whileHover={{ scale: 1.05, x: 5, y: 5 }} whileTap={{ scale: 0.95 }}
        ...
        <div className='flex items-center gap-4'>
          <motion.div variants={itemVars}
            ...
            <span className='text-primary'>{user?.codename ?? 'COMMANDER'}</span>
            <span className='opacity-30'>|</span>
            <span className='text-light-bronze'>LVL {levelInfo.level} {levelInfo.name}</span>
            <span className='opacity-30'>|</span>
            <span className='bg-soft px-2 py-0.5 border-2 border-black rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] text-xs md:text-sm'>{stats.xp} XP</span>
          </motion.div>

          <motion.button variants={itemVars} whileHover={{ scale: 1.05, x: 2, y: 2, boxShadow: 'none' }} whileTap={{ scale: 0.95 }}
            ...
            <LogOutIcon size={20} />
            <span className='hidden md:inline text-sm tracking-wider'>Log Out</span>
          </motion.button>
        </div>
      </div>
      ...
      <div className='max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8'>
        ...
        {/* SUCCESS RATE */}
        <motion.div variants={itemVars} whileHover={{ scale: 1.01 }}
        ...
        <div className='flex flex-col gap-6'>
          {[
            ...
            {
              label: 'STREAK', val: `${stats.streak} DAYS`, color: '#f07167',
              icon: <Image src='/Fire.gif' alt='streak fire' width={40} height={40} className={stats.streak === 0 ? 'opacity-100' : ''} unoptimized />,
            },
          ].map(item => (
            <motion.div key={item.label} variants={itemVars} whileHover={{ scale: 1.05, x: -5 }}
        ...
        <motion.div variants={itemVars} className='md:col-span-3 bg-white p-8 rounded-3xl border-4 border-black shadow-[10px_10px_0px_0px_rgba(0,0,0,1)]'>
          <h2 className='text-2xl mb-4 uppercase flex items-center gap-2'><ZapIcon size={24} /> Commander Rank</h2>
          <div className='flex items-center justify-between mb-3'>
            <span className='text-xl'>LVL {levelInfo.level} — {levelInfo.name}</span>
            <span className='text-sm opacity-60'>
              {levelInfo.nextLvl ? `${stats.xp} / ${levelInfo.nextLvl.minXP} XP → LVL ${levelInfo.level + 1}` : 'MAX LEVEL'}
            </span>
          </div>
          <div className='h-8 border-4 border-black rounded-xl bg-gray-100 overflow-hidden'>
            <motion.div initial={{ width: 0 }} animate={{ width: `${levelInfo.progress}%` }}
              transition={{ duration: 1.2, ease: 'easeOut' }} className='h-full bg-primary rounded-lg' />
          </div>
          <div className='flex justify-between mt-2 text-xs opacity-50 uppercase'>
            {XP_LEVELS.map(l => (
              <span key={l.level} className={l.level === levelInfo.level ? 'text-primary font-bold opacity-100' : ''}>{l.level}</span>
            ))}
          </div>
        </motion.div>
            ...
            {[...Array(7)].map((_, i) => {
              ...
              return (
                <div key={i} className='flex-1 h-full flex flex-col justify-end items-center gap-2'>
                  <motion.div
                    initial={{ height: 0 }} animate={{ height: `${Math.max(barHeight, dayData ? 8 : 0)}%` }}
        ...
        <motion.div variants={itemVars} className='md:col-span-3 bg-white p-8 rounded-3xl border-4 border-black shadow-[10px_10px_0px_0px_rgba(0,0,0,1)]'>
          ...
          <div className='flex items-end h-24 gap-1 border-b-2 border-black pb-1'>
            {[...Array(30)].map((_, i) => {
              ...
              const barHeight = dayData ? dayData.percent : 0;
              return (
                <motion.div key={i} initial={{ height: 0 }} animate={{ height: `${Math.max(barHeight, dayData ? 15 : 5)}%` }}
                  transition={{ delay: i * 0.02 }} style={getBarDecoration(dayData, barHeight, 'monthly')}
            ...
            {[...Array(52)].map((_, i) => (
              <div key={i} className='flex flex-col gap-1'>
                {[...Array(7)].map((_, j) => {
                  ...
                  return (
                    <motion.div key={j} whileHover={{ scale: 1.5, zIndex: 10 }}
        ...
        <motion.div variants={itemVars}
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            className='bg-white border-4 border-black p-4 rounded-xl shadow-[4px_4px_0px_0px_#000000] flex items-center gap-3 text-xl uppercase cursor-pointer transition-colors hover:bg-primary group'
          >
            <DownloadIcon size={24} />
            Download PDF
            <StarIcon size={16} className='text-yellow-500 group-hover:text-white' />
          </motion.button>
        </motion.div>
      </div>
    </motion.div>
