...
// Import your shared Navbar component here (adjust the path if needed)
...
export default function IntelDeckPage() {
  ...
  const updateNote = (fields: Partial<Note>) => {
    if (!activeId) return;
    setNotes((prev) =>
      prev.map((n) => (n.id === activeId ? { ...n, ...fields } : n)),
    );
  };
  ...
  const addImageFromUrl = () => {
    ...
    updateNote({
      imageUrls: [...(activeNote?.imageUrls || []), imageInput.trim()],
    });
    setImageInput('');
  ...
  const addImageFromFile = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      updateNote({
        imageUrls: [...(activeNote?.imageUrls || []), reader.result as string],
      });
    };
  ...
  const removeImage = (index: number) => {
    updateNote({
      imageUrls: activeNote?.imageUrls.filter((_, i) => i !== index) ?? [],
    });
  };
  ...
  const toggleFormat = (cls: string) => {
    ...
    updateNote({
      textDecoration: cur.includes(cls)
        ? cur.replace(cls, '').trim()
        : `${cur} ${cls}`.trim(),
    });
  };
  ...
  return (
    <div className='w-screen h-screen bg-[#fefae0] flex flex-col overflow-hidden font-luckiest select-none relative'>
      {/* ── SHARED NAVBAR ── */}
      <Navbar
      ...
      {/* ── WORKSPACE AREA (Takes remaining flexible space) ── */}
      <div className='w-full flex-1 flex overflow-hidden relative'>
        ...
        <AnimatePresence>
          {view === 'canvas' && showSidebar && (
            <motion.aside
              ...
              className='h-full border-r-4 border-black bg-white flex flex-col overflow-hidden shrink-0 z-20'
            >
              {/* Search */}
              ...
              <div className='flex-1 overflow-y-auto p-3 space-y-1.5'>
                {filteredNotes.length === 0 && (
                  <p className='text-[10px] uppercase opacity-30 text-center py-8'>
                    No notes yet
                  </p>
                )}
                {filteredNotes.map((n) => (
              ...
              {/* New note button */}
              <div className='p-3 border-t-2 border-black/10 shrink-0 bg-white'>
                <motion.button
                  whileTap={{ scale: 0.96 }}
                  onClick={newNote}
        ...
        {/* MAIN DISPLAY AREA */}
        <main className='flex-1 h-full overflow-hidden flex flex-col relative bg-[#fefae0]'>
          {/* Sub-header inside workspace to toggle between Canvas / Vault and controls */}
          <div className='w-full px-5 py-2.5 border-b-4 border-black bg-white flex items-center justify-between shrink-0 z-10'>
            <div className='flex items-center gap-3'>
              <Link href='/board'>
                <motion.div
                  whileHover={{ scale: 1.04, x: 2, y: 2, boxShadow: 'none' }}
                  whileTap={{ scale: 0.96 }}
                  className='flex items-center gap-1.5 bg-[#e9edc9] border-2 border-black px-3 py-1 rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] cursor-pointer text-xs uppercase'
                >
                  <ArrowLeftIcon size={14} /> Back
                </motion.div>
              </Link>
              <h2 className='text-lg font-oi uppercase tracking-tight'>
                Intel Deck
              </h2>
            </div>

            {/* View toggle */}
            <div className='flex bg-[#fefae0] border-2 border-black rounded-xl overflow-hidden shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]'>
              <button
                onClick={() => setView('canvas')}
                className={`px-3 py-1.5 text-xs uppercase flex items-center gap-1.5 transition-all cursor-pointer ${view === 'canvas' ? 'bg-[#ccd5ae]' : 'hover:bg-[#e9edc9]'}`}
              >
                <FileTextIcon size={13} /> Canvas
              </button>
              <button
                onClick={() => setView('vault')}
                className={`px-3 py-1.5 text-xs uppercase flex items-center gap-1.5 transition-all cursor-pointer border-l-2 border-black ${view === 'vault' ? 'bg-[#faedcd]' : 'hover:bg-[#e9edc9]'}`}
              >
                <LayoutGridIcon size={13} /> Vault{' '}
              ...
              >
                {!activeNote ? (
                  <div className='w-full h-full flex items-center justify-center'>
                    <Empty
                      label='Create a new note to start writing'
                      onAdd={newNote}
                    />
                  </div>
                ) : (
                  <>
                    {/* Inner Canvas Toolbar */}
                    <div className='flex items-center justify-between px-5 py-2.5 border-b-2 border-black/10 bg-white shrink-0 z-10'>
                      <div className='flex items-center gap-2'>
                        ...
                        <motion.button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 border-2 border-black rounded-xl cursor-pointer text-xs uppercase transition-colors ${showFormat ? 'bg-[#faedcd]' : 'bg-white hover:bg-[#fefae0]'}`}
                        >
                        ...
                        <div className='flex items-center gap-1'>
                          <motion.button
                            ...
                            className='flex items-center gap-1.5 px-3 py-1.5 border-2 border-black rounded-xl cursor-pointer text-xs uppercase bg-white hover:bg-[#fefae0] transition-colors'
                          >
                      ...
                      <div className='flex items-center gap-2'>
                        {activeNote.audioUrl ? (
                          <div className='flex items-center gap-2 bg-[#fefae0] border-2 border-black rounded-xl px-2.5 py-1'>
                            <audio
                          ...
                          <motion.button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 border-2 border-black rounded-xl cursor-pointer text-xs uppercase transition-colors ${isRecording ? 'bg-red-500 text-white animate-pulse' : 'bg-white hover:bg-[#ffadad]'}`}
                          >
                            {isRecording ? (
                              <>
                      ...
                      {showImageInput && (
                        ...
                        >
                          ...
                          <motion.button
                            ...
                            className='bg-[#d4a373] border-2 border-black px-4 py-2 rounded-xl text-xs uppercase cursor-pointer'
                          >
                            Add
                          </motion.button>
                          <button
                        ...
                        >
                          <div className='flex gap-1'>
                            {[
                              ...
                              {
                                icon: <StrikethroughIcon size={13} />,
                                cls: 'line-through',
                              },
                            ].map((f, i) => (
                              <button
                                key={i}
                                onClick={() => toggleFormat(f.cls)}
                          ...
                          <div className='flex gap-1'>
                            {(
                              ['font-sans', 'font-mono', 'font-serif'] as const
                            ).map((f) => (
                              <button
                                key={f}
                                onClick={() => updateNote({ fontStyle: f })}
                          ...
                          <div className='flex items-center gap-2'>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updateNote({ color: e.target.value })
                              }
                          ...
                          <div className='flex items-center gap-2'>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updateNote({ textColor: e.target.value })
                              }
                              className='w-7 h-7 rounded-lg border-2 border-black cursor-pointer'
                            />
                          </div>
                    ...
                    {/* Actual Scrolling Document Content Area */}
                    <div
                    ...
                    >
                      <div className='max-w-3xl mx-auto px-8 py-8 space-y-5'>
                        <input
                          ...
                          onChange={(e) =>
                            updateNote({ title: e.target.value.toUpperCase() })
                          }
                        ...
                        <p className='text-[10px] opacity-30 uppercase font-sans'>
                          {activeNote.createdAt}
                        </p>

                        {activeNote.imageUrls.length > 0 && (
                          <div
                            className={`grid gap-3 ${activeNote.imageUrls.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}
                          >
                            {activeNote.imageUrls.map((url, i) => (
                              <div
                                key={i}
                                className='relative group border-4 border-black rounded-2xl overflow-hidden shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] max-h-72'
                              >
                                <img
                                  src={url}
                                  alt=''
                                  ...
                                  onError={(e) => {
                                    (e.target as HTMLImageElement).src =
                                      'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="400" height="200"><rect fill="%23e9edc9" width="400" height="200"/><text fill="%23999" font-size="14" x="50%" y="50%" text-anchor="middle" dy=".3em">Image not found</text></svg>';
                                  }}
                                />
                                <button
            ...
            {view === 'vault' && (
              ...
              >
                  ...
                  {showNewAsset && (
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewAsset((p) => ({ ...p, title: e.target.value }))
                        }
                ...
                {filteredAssets.length === 0 ? (
                  <div className='h-64'>
                    <Empty
                      label='Drop links, code snippets, and notes into your vault'
                      onAdd={() => setShowNewAsset(true)}
                    />
                  </div>
                ) : (
                    ...
                    <AnimatePresence>
                      {filteredAssets.map((a) => (
                        ...
                        >
                          ...
                          <motion.button
                            whileTap={{ scale: 0.95 }}
                            onClick={() => copyAsset(a.content, a.id)}
