    ...
    <div className='min-h-screen bg-soft text-foreground font-luckiest overflow-x-hidden'>
      ...
      <div className='flex items-center justify-between px-6 md:px-10 pt-6 md:pt-8'>
              ...
              {isLoggedIn ? (
                <Link href='/user'>
                  <motion.div
                    ...
                    whileTap={{ scale: 0.96 }}
                    className='flex items-center gap-2 bg-white border-4 border-black px-4 py-2.5 rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] cursor-pointer group'
                  >
                    <UserIcon
                      size={20}
        ...
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className='text-xl md:text-2xl text-primary tabular-nums'
        >
          {currentTime}
        </motion.div>
      </div>
      ...
      <div className='text-center px-6 pt-8 pb-6'>
        <motion.h1
          ...
          animate={{ opacity: 1, y: 0 }}
          className='text-4xl md:text-5xl font-oi tracking-wide uppercase mb-3'
        >
          QuestBoard
        </motion.h1>
        ...
        <motion.div
          ...
          className='flex flex-col md:flex-row items-center justify-center gap-2 md:gap-6 text-light-bronze'
        >
          <span className='flex items-center gap-2 text-lg md:text-xl'>
            <CalendarDaysIcon size={20} /> {date}
          </span>
          <span className='hidden md:block opacity-30'>·</span>
          <span className='flex items-center gap-2 text-sm md:text-base opacity-70'>
      ...
      <AnimatePresence>
        {isLoggedIn && totalCount > 0 && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0 }}
            className='max-w-4xl mx-auto px-6 md:px-10 mb-4'
          >
            <div className='flex items-center justify-between text-sm opacity-60 uppercase mb-1.5'>
              <span>
      ...
      <div className='max-w-6xl mx-auto px-6 md:px-10 pb-32'>
        <div className='border-4 border-black rounded-3xl overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-white'>
          ...
          {/* ADD ROW — inline at bottom of table */}
          <motion.button
            whileHover={{ backgroundColor: '#f5f0e8' }}
            whileTap={{ scale: 0.99 }}
            onClick={addTask}
            className='w-full flex items-center gap-3 px-5 py-4 border-t-4 border-black bg-white text-black/40 hover:text-black/70 transition-colors cursor-pointer'
          >
            <Plus size={16} />
            <span className='text-sm uppercase tracking-widest'>Add quest</span>
          </motion.button>
        </div>
      </div>
          ...
          >
            <Link href='/dashboard'>
              <motion.div
                ...
                whileTap={{ scale: 0.95 }}
                className='flex items-center gap-2 bg-white border-4 border-black px-4 py-3 rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] cursor-pointer group'
              >
                <LayoutDashboardIcon
                  size={22}
                  className='group-hover:rotate-12 transition-transform'
                />
                <span className='text-base uppercase'>Dashboard</span>
              </motion.div>
            </Link>
          </motion.div>
        )}
