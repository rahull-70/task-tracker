...
export default function LandingPage() {
  ...
  return (
    <div className='min-h-screen font-luckiest overflow-x-hidden'>
      {/* NAV — white */}
      <nav className='fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b-4 border-black select-none text-black font-luckiest'>
      <div className='max-w-6xl mx-auto flex items-center justify-between px-2 sm:px-6 py-3 min-h-[64px] gap-2'>
        
        {/* BRAND LOGO - Fluid sizing to prevent mobile text-wrapping */}
        <Link href="/" className="flex-shrink-0">
          <span className='text-lg sm:text-xl md:text-2xl font-oi uppercase tracking-wide block leading-none'>
            QuestBoard
          </span>
        </Link>
        
        {/* DESKTOP LINKS - Stays hidden until md breakpoint */}
        <div className='hidden md:flex items-center gap-5 text-sm uppercase opacity-50'>
          <Link href='#features' className='hover:opacity-100 transition-opacity cursor-pointer'>
            Features
          </Link>
          <Link href='/board' className='hover:opacity-100 transition-opacity cursor-pointer'>
            Dashboard
          </Link>
          <Link href='/calendar' className='hover:opacity-100 transition-opacity cursor-pointer'>
            Calendar
          </Link>
          <Link href='/plans' className='hover:opacity-100 transition-opacity cursor-pointer'>
            Plans
          </Link>
          <Link href='#pricing' className='hover:opacity-100 transition-opacity cursor-pointer'>
            Pricing
          </Link>
        </div>
        
        {/* CTAS - Forced horizontal layout with smaller scaling on small screens */}
        <div className='flex items-center gap-2 sm:gap-3 flex-shrink-0'>
          <Link href='/login'>
            <motion.div
              whileHover={{ scale: 1.04 }}
              whileTap={{ scale: 0.96 }}
              className='px-2.5 sm:px-4 py-1.5 sm:py-2 border-4 border-black rounded-xl bg-[#fefae0] shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] text-[11px] sm:text-sm uppercase cursor-pointer whitespace-nowrap text-center leading-none'
            >
              Sign In
            </motion.div>
          </Link>
          ...
          <Link href='/sign-in'>
            <motion.div
              ...
              whileTap={{ scale: 0.96 }}
              className='px-2.5 sm:px-4 py-1.5 sm:py-2 border-4 border-black rounded-xl bg-[#d4a373] shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] text-[11px] sm:text-sm uppercase cursor-pointer whitespace-nowrap text-center leading-none'
            >
              Get Started
            </motion.div>
          </Link>
     ...
     <section
      ref={heroRef}
      className='min-h-screen bg-[#fefae0] flex flex-col items-center justify-center text-center px-4 sm:px-6 pt-28 pb-16 relative overflow-hidden select-none text-black font-luckiest'
    >
      {/* Decorative blobs */}
      ...
      <motion.div style={{ y }} className='relative z-10 w-full max-w-4xl mx-auto flex flex-col items-center'>
        ...
        >
          <Badge bg='bg-white'>
            <ZapIcon size={12} className="fill-current" /> Track · Complete · Level Up
          </Badge>
        </motion.div>
        ...
        <motion.h1
          ...
          className='text-5xl sm:text-7xl md:text-8xl lg:text-[10rem] font-oi uppercase leading-[0.9] mt-6 mb-6 tracking-tight max-w-full break-words'
        >
          Quest
          <br />
          Board
        ...
        <motion.p
          ...
          className='text-sm sm:text-base md:text-xl font-sans opacity-70 max-w-md md:max-w-lg mx-auto mb-10 leading-relaxed px-2'
        >
          Turn your daily tasks into missions. Earn XP, build streaks, and
          level up your productivity — one quest at a time.
        </motion.p>

        {/* MOBILE-RESPONSIVE CALL TO ACTIONS */}
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.4 }}
          className='flex flex-col sm:flex-row gap-4 justify-center w-full max-w-sm sm:max-w-none px-4 sm:px-0'
        >
          <Link href='/sign-in' className="w-full sm:w-auto">
            <motion.div
              whileHover={{ scale: 1.03, x: 3, y: 3, boxShadow: 'none' }}
              whileTap={{ scale: 0.97 }}
              className='flex items-center justify-center gap-2 bg-[#d4a373] border-4 border-black px-8 py-4 rounded-2xl shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] text-lg md:text-xl uppercase cursor-pointer transition-all active:translate-x-0.5 active:translate-y-0.5 active:shadow-none'
            >
          ...
          <Link href='/board' className="w-full sm:w-auto">
            <motion.div
              whileHover={{ scale: 1.03, x: 3, y: 3, boxShadow: 'none' }}
              whileTap={{ scale: 0.97 }}
              className='flex items-center justify-center gap-2 bg-white border-4 border-black px-8 py-4 rounded-2xl shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] text-lg md:text-xl uppercase cursor-pointer transition-all active:translate-x-0.5 active:translate-y-0.5 active:shadow-none'
            >
              Open App
            </motion.div>
          </Link>
      ...
      {/* RESTORED STATS ROW WITH FLUID MOBILE ADAPTABILITY */}
      <motion.div
        initial={{ opacity: 0, y: 40 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.65 }}
        className='relative z-10 mt-12 grid grid-cols-3 gap-2.5 sm:gap-4 w-full max-w-md md:max-w-lg mx-auto px-2 sm:px-0'
      >
        {[
          ['10K+', 'Quests Done'],
          ['500+', 'Commanders'],
          ...
          <div
            key={i}
            className='bg-white border-4 border-black rounded-xl sm:rounded-2xl p-2 sm:p-4 text-center shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] sm:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col justify-center items-center min-w-0'
          >
            <p className='text-base xs:text-lg sm:text-2xl md:text-3xl font-luckiest leading-none truncate w-full'>
              {val}
            </p>
            <p className='text-[8px] sm:text-[10px] opacity-60 uppercase tracking-wide font-sans mt-1 font-bold line-clamp-2 sm:line-clamp-none'>
              {label}
            </p>
          </div>
        ))}
      ...
      <motion.p
        ...
        className='absolute bottom-6 left-1/2 -translate-x-1/2 text-xs uppercase opacity-40 tracking-widest hidden md:block'
      >
        Scroll to explore ↓
      </motion.p>
    </section>
